...
function buildPlaceholderDay(day) {
    const difficulty = day < 10 ? 'beginner' : day < 20 ? 'familiar' : 'intermediate';
    return {
        day,
...
async function run() {
    ...
    const { data: existingPlans, error: lookupError } = await supabase_1.supabaseAdmin
        .from('daily_plans')
        .select('id')
    ...
    const { error: insertError } = await supabase_1.supabaseAdmin.from('daily_plans').upsert({
        id: DEMO_PLAN_ID,
