...
const heatmapService = new heatmap_service_1.HeatmapService();
 ...
 */
async function checkPerfectDay(userId, planId, date) {
    const dateStart = new Date(date);
    dateStart.setHours(0, 0, 0, 0);
    const dateEnd = new Date(dateStart.getTime() + 24 * 60 * 60 * 1000);
    const { data: xpEvents } = await supabase_1.supabaseAdmin
        .from('xp_events')
        .select('reason')
        .eq('user_id', userId)
...
async function updateStreak(userId) {
    const prefs = await supabase_1.supabaseAdmin
    ...
    const lastActive = prefs.data.last_active_date;
    const currentStreak = prefs.data.streak_count || 0;
    const longestStreak = prefs.data.longest_streak || 0;
    const freezeLastUsed = prefs.data.freeze_last_used;
    const freezeCount = prefs.data.freeze_count || 1;
...
router.post('/complete-task', async (req, res) => {
    ...
    try {
        ...
        const planData = await supabase_1.supabaseAdmin
            .from('daily_plans')
            .select('id')
            .eq('user_id', userId)
            .eq('status', 'active')
        ...
        const awardedBadges = [];
        // First task badge
        const { data: firstTaskData } = await supabase_1.supabaseAdmin
            .from('xp_events')
            .select('id')
            .eq('user_id', userId)
        ...
        if (room_id && typeof room_id === 'string') {
            ...
            if (currentTasksDone === 3) {
                ...
                if (!existingPosition) {
                    ...
                    const { data: roomMembers } = await supabase_1.supabaseAdmin
                        .from('room_members')
                        .select('user_id')
                        .eq('room_id', room_id);
                    const memberCount = roomMembers?.length || 0;
                    const { data: completedToday } = await supabase_1.supabaseAdmin
                        .from('room_daily_log')
                        .select('user_id')
                        .eq('room_id', room_id)
                        .eq('date', today)
...
router.post('/submit-practice', async (req, res) => {
    ...
    try {
        const { plan_id, day_number, passed, submitted_code, error_type, hint_used, room_id, } = req.body;
        ...
        if (passed) {
            ...
            const { data: firstPracticeData } = await supabase_1.supabaseAdmin
            ...
            const { count: practiceCount } = await supabase_1.supabaseAdmin
                .from('practice_attempts')
            ...
            let isPerfectDay = false;
            if (planData.data) {
                isPerfectDay = await checkPerfectDay(userId, plan_id, new Date().toISOString().slice(0, 10));
                if (isPerfectDay) {
                    await heatmapService.logContributionEvent({
                        userId,
                        eventType: 'perfect_day',
...
router.post('/busy-day', async (req, res) => {
    ...
    try {
        ...
        const prefs = await supabase_1.supabaseAdmin
            ...
            .eq('user_id', userId)
            .single();
        if (!prefs.data || prefs.data.freeze_count === 0) {
        ...
        if (evalResult.passed) {
            ...
            if (evalResult.passed && room_id) {
                try {
                    ...
                    const { data: existingLog } = await supabase_1.supabaseAdmin
                        .from('room_daily_log')
                        .select('tasks_done, xp_earned, started_at')
                        .eq('room_id', room_id)
                        .eq('user_id', userId)
                    ...
                    console.log(`[Room] Updating room_daily_log: ` +
