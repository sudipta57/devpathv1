...
function makeChain(singleResult, insertResult) {
    return {
        ...
        insert: jest.fn().mockReturnValue({
            select: jest.fn().mockReturnThis(),
            single: jest.fn().mockResolvedValue(insertResult || { data: { id: 'new-id' }, error: null }),
        }),
        count: 0,
    };
}
...
describe('GET /api/mission/today', () => {
    beforeEach(() => {
        ...
        mockFrom.mockImplementation((table) => {
            if (table === 'daily_plans') {
                return makeChain({ data: ACTIVE_PLAN, error: null });
            }
            if (table === 'user_preferences') {
                return makeChain({ data: USER_PREFS, error: null });
            }
            if (table === 'xp_events') {
                return {
                    select: jest.fn().mockReturnThis(),
    ...
    test('returns 401 without x-user-id', async () => {
        const res = await (0, supertest_1.default)(app_1.default).get('/api/mission/today');
...
describe('POST /api/mission/complete-task', () => {
    ...
    test('returns 200 with xp_awarded=20 for task 1', async () => {
        ...
        expect(res.status).toBe(200);
        expect(mockAwardXp).toHaveBeenCalledWith(USER_ID, xp_service_1.XP.TASK_COMPLETE, 'task1_complete');
    });
    test('returns 200 with xp_awarded=20 for task 2', async () => {
        ...
        expect(res.status).toBe(200);
        expect(mockAwardXp).toHaveBeenCalledWith(USER_ID, xp_service_1.XP.TASK_COMPLETE, 'task2_complete');
    });
...
describe('POST /api/mission/submit-practice', () => {
    ...
    test('awards 45 XP on pass without hint (30 + 15 no-hint bonus)', async () => {
        ...
        expect(mockAwardXp).toHaveBeenCalledWith(USER_ID, xp_service_1.XP.PRACTICE_SOLVED + xp_service_1.XP.NO_HINT_BONUS, 'practice_solved_no_hint');
    });
    test('awards 30 XP on pass with hint used', async () => {
        ...
        expect(res.status).toBe(200);
        expect(mockAwardXp).toHaveBeenCalledWith(USER_ID, xp_service_1.XP.PRACTICE_SOLVED, 'practice_solved');
    });
...
describe('POST /api/mission/busy-day', () => {
    ...
    test('uses freeze, preserves streak, awards 5 XP', async () => {
        ...
        expect(mockAwardXp).toHaveBeenCalledWith(USER_ID, xp_service_1.XP.BUSY_DAY, 'busy_day');
    });
...
describe('POST /api/mission/skip-day', () => {
    ...
    test('returns 401 without x-user-id', async () => {
        const res = await (0, supertest_1.default)(app_1.default).post('/api/mission/skip-day');
        expect(res.status).toBe(401);
    });
});
