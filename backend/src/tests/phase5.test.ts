...
import { getMicroLesson, isQuotaError } from '../services/gemini.service';
import request from 'supertest';
import app from '../app';

const mockFrom = supabaseAdmin.from as jest.MockedFunction<typeof supabaseAdmin.from>;
const mockGetMicroLesson = getMicroLesson as jest.MockedFunction<typeof getMicroLesson>;
const mockIsQuotaError = isQuotaError as jest.MockedFunction<typeof isQuotaError>;
...
function makePrefsChain(data: unknown) {
    return {
...
function setupMocks({
    prefs = BASE_PREFS,
    attempts = [],
    microLesson = 'Step 1: Check your syntax.',
}: {
    prefs?: unknown;
    attempts?: unknown[];
    microLesson?: string;
    ...
    mockFrom.mockImplementation((table: string) => {
        if (table === 'user_preferences') return makePrefsChain(prefs) as unknown as ReturnType<typeof supabaseAdmin.from>;
...
describe('POST /api/mission/stuck', () => {
    ...
    test('returns 401 without x-user-id header', async () => {
        const res = await request(app).post('/api/mission/stuck').send(VALID_BODY);
    ...
    test('uses "unknown error" when attempts have no error_type', async () => {
        ...
        expect(mockGetMicroLesson).toHaveBeenCalledWith(
            expect.objectContaining({ errorTypes: ['unknown error'] })
        );
    });
    ...
    test('inserts a practice_attempts row with hint_used=true', async () => {
        ...
        expect(insertMock).toHaveBeenCalledWith(
            expect.objectContaining({ hint_used: true, passed: false })
        );
    });
