...
function runtimeUuid(seedPrefix: number): string {
...
async function checkMaterializedView(): Promise<void> {
...
async function checkContributionTrigger(): Promise<void> {
...
async function checkXpTrigger(): Promise<void> {
  await ensureTestUser();
...
async function checkRealtimePublication(): Promise<void> {
  ...
  try {
    ...
    const queryResult = await client.query<{ tablename: string }>(
      `
      SELECT tablename
...
async function checkDemoAccount(): Promise<void> {
...
async function checkDemoRoom(): Promise<void> {
  const { data, error } = await supabaseAdmin
    .from('rooms')
    .select('id,status')
    .eq('code', 'KGEC42')
...
async function checkDemoPlan(): Promise<void> {
  const { data, error } = await supabaseAdmin
    .from('daily_plans')
    .select('id')
