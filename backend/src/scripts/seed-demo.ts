...
function buildRecentDays(count: number): string[] {
...
async function seedDemoUser(todayIso: string): Promise<void> {
...
async function seedContributionHistory(): Promise<void> {
  ...
  const { error } = await supabaseAdmin.from('contribution_events').upsert(rows, {
    onConflict: 'id',
  });
...
async function seedXpHistory(): Promise<void> {
  ...
  const { error } = await supabaseAdmin.from('xp_events').upsert(xpRows, {
    onConflict: 'id',
  });
...
async function seedBadges(): Promise<void> {
  ...
  const { error } = await supabaseAdmin.from('badges').upsert(rows, {
...
async function seedRoom(todayIso: string): Promise<void> {
  ...
  const { error: fakeUsersError } = await supabaseAdmin.from('users').upsert(userRows, {
    onConflict: 'id',
  });
  ...
  if (existingRoom?.id) {
    ...
    const { error: insertRoomError } = await supabaseAdmin.from('rooms').insert({
      id: DEMO_ROOM_ID,
      code: 'KGEC42',
      name: 'KGEC DSA Squad',
      type: 'daily_sprint',
  ...
  const { error: membersError } = await supabaseAdmin.from('room_members').upsert(memberRows, {
  ...
  const { error: dailyLogError } = await supabaseAdmin.from('room_daily_log').upsert(dailyLogRows, {
  ...
  const { error: roomEventsError } = await supabaseAdmin.from('room_events').upsert(eventRows, {
    onConflict: 'id',
  });
...
async function seedPracticeAttempts(): Promise<void> {
  ...
  const { error } = await supabaseAdmin.from('practice_attempts').upsert(attempts, {
    onConflict: 'id',
  });
